...
- [x] **Step 2: 运行两个测试文件并观察缺少评分 API 的失败**
...
- [x] **Step 1: 写 Repository 行为失败测试**

新增六个测试：编辑 L2 保留不可变字段并重载成功；编辑 L3 重载成功；删除目标后重载仍不存在且不存在 ID 报错；并发编辑/删除后 `flush()` 与重载状态一致；心跳等待提炼期间修正或删除 L2 时丢弃旧候选。
...
- [x] **Step 2: 运行记忆测试并确认方法缺失**

Run: `npx tsx --test --test-isolation=none tests/memory-engine.test.ts`

Expected: FAIL，`updateMemory`/`deleteMemory` 尚不存在。

- [x] **Step 3: 添加最小 Repository 与 Engine 实现**

Repository 按 `tier` 选择 `database.l2` 或 `database.l3`，按 ID 原位更新或 `splice()` 删除；未找到时抛出 `没有找到要修改的记忆`/`没有找到要删除的记忆`。更新时间使用 ISO 字符串，摘要调用 `summarizeText(content)`，重要度调用 `clamp()`，随后 `await persist()`。`MemoryEngine.consolidate()` 在 reflector 返回后比较来源 ID、更新时间、内容、类型和重要度，任一来源变化即返回 0，不写入过期 L3 候选。

Engine 只转发 Repository 写操作并返回最新 `snapshot()`，不触碰 L1。

- [x] **Step 4: 运行记忆测试和全部单元测试**
...
- [x] **Step 1: 先修改 bridge 类型，运行类型检查观察实现缺口**
...
- [x] **Step 2: 接通 preload 与主进程受限 handler**
