...
flowchart LR
  ...
  Main --> Capability
  Capability -->|固定参数 + 审批| DesktopTools[浏览器 / 剪贴板 / 保存窗口 / 白名单应用 / Office]
  Capability -->|已审批命令队列| BrowserBridge
...
- `AgentToolRuntime`：注册记忆搜索/明确保存、人格/关系读取、一次性桌面感知、网页/本机/Office 操作预览、2～4 步协作计划和桌宠动作；逐项解析 JSON、限制调用次数、再次校验本机参数、收集可见 trace，并把工具结果回填模型。工具端点不兼容时保留普通聊天与确定性单句/复合句兜底。
- `BrowserContextServer`：在回环地址上同时处理用户主动提交的网页上下文，以及经桌宠审批后由扩展领取的一次性刷新、前进/后退、向上/下滚动、滚到顶部/底部和页内查找命令；令牌、扩展 Origin、队列、时限和结果提交都在服务端校验。
- `OfficeAutomationService`：用固定 PowerShell COM 脚本连接当前活动 Word/Excel/PowerPoint；用户数据只走标准输入的 Base64(JSON)，提供有界纯文本写入和稳定错误映射，不自动保存。
- `companion-dialogue`：纯逻辑地判断倾诉/闲聊/信息/回忆等对话节奏，构建“住在桌面且有 Live2D 数字身体”的有限陪伴契约，恢复并兼容清洗旧 L1 助手轮次；用户可见正文在写记忆、情绪推断和 TTS 前移除舞台式动作/情绪标签，将笑声改为自然口语，同时保留普通括号、引用和代码。
...
- `ModelStore`：从公共 `bundled-models` 注册表加载 6 套官方样例（Hiyori/Mao/Wanko/Haru/Mark/Rice）与 3 套第三方授权模型（Nana/Cyannyan/小云），携带来源和低置信气质元数据；在主进程校验并复制用户选择的 Cubism `.model3.json` 资源，持久化当前模型，并向宠物窗口返回不含本地路径的受限资源包。
...
- `ComputerProposalQueue`：Renderer 的纯顺序审批状态机；用 revision ticket 隔离旧步骤的迟到结果，新计划替换时清理旧 pending，只有 `completed` 才推进，拒绝、取消、失败、过期或 IPC 异常都会停止余下步骤。
...
- `live2d-interaction`：不依赖 DOM/Pixi 的真实参数能力绑定、焦点阻尼、拖拽/下落/落地变形、18 项动作映射、资源时长解析与平滑程序化全身/参数兜底。
- `PetReactionDirector`：在 Renderer 内把回复情绪和文本映射为至多一个动作；处理强动作冷却、录音/拖拽优先级、最新待执行动作和手动预览优先级。
- `Live2DPetAdapter`：使用 PixiJS 8、Cubism Framework 与官方 Core 5.1 渲染内置和用户导入模型，处理真实索引参数写入、选择性语义 motion、逐模型情绪 expression、标准口型兜底、无嘴模型发言脉冲、自动取景、视线、物理、弹簧变形与模型资源释放。
- `DefaultPetAdapter`：Live2D 加载或 WebGL 初始化失败时使用的轻量程序化后备模型，保证聊天和桌面交互仍可用。
- `PetModelAdapter`：模型渲染边界，使 Agent、桌面移动和具体 2D Runtime 彼此解耦。
- `BrowserContextServer`：只监听 `127.0.0.1:32145` 的回环接口；校验随机配对令牌、扩展 Origin、请求大小、八类浏览器动作和 http(s) 页面来源，承载用户手势上下文以及一次性命令领取/结果回传，不返回记忆或本机状态。
- `ComputerCapabilityController`：白名单草稿二次清洗、2～4 步计划原子创建与主进程顺序校验、同一审批 ID 单次消费、不可变 pending 参数、会话/持久权限、重启/过期收尾、真实执行状态与最多 500 条本地审计；Renderer 只能提交操作 ID 和授权决定。
- `computer-work-plan`：定义模型计划的固定步骤枚举，解析 2～4 个步骤并把各类参数收敛为既有 `ComputerActionDraft`，不创建新执行原语。
...
普通聊天在已有陪伴契约和自动相关记忆之外，向支持 Chat Completions function calling 的端点发送十五项工具定义。Provider 保留标准 assistant `tool_calls`，Runtime 执行后追加匹配的 `role: tool / tool_call_id`，最多四轮后得到最终自然语言回复；单轮实际调用总数限制为十次。模型可以并行请求多个只读工具，但 Runtime 每轮只接受一个电脑单步预览或一个 2～4 步协作计划、一次桌面感知和一次明确记忆写入。
...
- 单步 `computer_*` 只调用 `ComputerCapabilityController.planDraft()` 创建不可变 pending 参数；`computer_work_plan` 把 2～4 个既有白名单草稿交给 `planDrafts()` 整体清洗和权限预检，再附加同组计划 ID 与步骤序号。真正执行仍等待 Renderer 只回传 UUID 和授权决定。Renderer 一次展示一步，当前步骤完成才展示下一步；拒绝、取消、失败或过期会停止余下 pending 步骤，已经完成的外部操作不做自动撤回。浏览器控制通过扩展领取单次命令，Office 通过固定 COM 服务连接当前活动文档；二者均不支持会话或长期免确认。执行结果进入本机审计，并以 `computer-tool-result` 写入 L1 供下一轮承接。
- `pet_action` 只返回一个语义动作请求，Renderer 仍通过 `PetReactionDirector` 应用录音、移动、手动动作和强动作冷却优先级。
- 若端点拒绝 `tools` 字段或没有完成工具协议，AgentService 会重试一次不带工具的普通兼容聊天；明确记忆和常见中文电脑意图继续由本机确定性规则兜底。
...
正式 Electron 页面不监听 Renderer 局部 `pointermove` 作为视线源。主进程每 33ms 读取 `screen.getCursorScreenPoint()`，以窗口中模型视觉中心为原点，使用水平 640px、垂直 480px 半径归一化到 `[-1,1]`；浏览器预览也通过 mock bridge 复用 `onPetFocus`。Live2D 加载后枚举 moc 的真实参数索引，只绑定存在的 `ParamEyeBallX/Y`、`ParamAngleX/Y/Z`、`ParamBodyAngleX/Y/Z` 或旧式 `PARAM_ANGLE_* / PARAM_BODY_ANGLE_* / PARAM_EAR_L/R`，并用 `addParameterValueByIndex()` 追加阻尼值。没有眼球 XY 的模型会提高头身权重，Wanko 的双耳获得相反方向的轻微反馈。

## 电脑协作与授权

电脑协作总开关默认关闭。开启后有三种显式上下文来源：浏览器扩展右键菜单、用户已复制的剪贴板文本、原生文件选择器选中的受限文本文件。浏览器请求通过随机 32 字节 base64url 令牌配对；HTTP 服务只绑定回环地址，CORS 只回显 `chrome-extension://` / `moz-extension://` 来源，正文限制 64KB、实际上下文限制 12000 字。扩展没有读取记忆、设置、文件或页面数据的端点；只有用户在扩展中另行开启接收后，才能长轮询领取已经在桌宠审批的一次性固定命令。重置令牌会即时撤销旧扩展访问。
...
普通聊天中的单步工作请求优先由模型通过对应 `computer_*` function tool 提交，明确的连续工作可使用 `computer_work_plan`；离线模式、只支持文本的兼容端点或模型漏调工具时，确定性中文规划器继续兜底。它会先尝试把含“然后 / 接着 / 随后 / 再”或分号的复合句拆成 2～4 段，只有全部段落都能匹配白名单动作时才形成计划，否则不生成部分计划。单步路径进入 `ComputerCapabilityController.planDraft()`，计划路径进入 `planDrafts()`；两者都会再次清洗 http(s) URL、文本、文件名、应用枚举、浏览器动作、A1 单元格和 Office 载荷。控制器保存完整参数，Renderer 得到的只是裁剪预览、随机 UUID 和计划序号；确认时只回传 UUID 与授权决定，因此确认后参数不会被 Renderer 或模型替换。

`save-text-file` 不接受会话或长期许可，并始终经过原生保存对话框；`launch-app` 只允许记事本、计算器和用户主目录资源管理器。`browser-control` 只允许当前活动 http(s) 标签页的刷新、前进/后退、向上/下滚动、滚到顶部/底部和页内查找，使用最多 8 项、45 秒过期的一次性队列。`office-write` 只允许当前活动 Word 追加纯文本、Excel 从 A1 地址写入纯文本矩阵、PowerPoint 新增标题/正文页；固定脚本不拼接用户文本、不主动调用宏、不打开路径且不自动保存。Excel 写入期间暂时关闭并恢复 `EnableEvents`；Word/PowerPoint 已有文档事件仍可能由 Office 自身响应。2～4 步计划只编排这些现有原语，不提供条件分支、循环、后台继续或自动回滚；当前也不提供 Shell、任意路径读写、CDP 接管或后台通用输入模拟。
...
用户通过原生目录选择器选择 Live2D 文件夹。`ModelStore` 只接受一个 `Version: 3` 的 `.model3.json`，要求存在其引用的 `.moc3` 与贴图，并收集可选的 motion、expression、physics、pose、userdata、display info 和动作音频。它会拒绝路径穿越、绝对路径、符号链接、未知扩展名、缺失引用和超限资源。当前 Runtime 固定 Cubism Core 5.1，资源回归要求 moc 文件头为 `MOC3` 且版本字节不高于 5；Core 6/v6 模型必须从原工程重新导出为 Cubism 5 兼容资源。有效文件会复制到 `data/models/imported/<id>/`；manifest 保存公开模型元数据、`user-import` 来源和资源白名单，`data/models/model-state.json` 只记录当前模型 ID。导入模型没有预置气质，继续使用中性起点。
...
3. 一次性动作：`wave / nod / shake-head / head-tilt / jump / cheer / dance / sit / stretch / shy / comfort / sleep / surprised / bow / applaud / peek / ponder / present`，由白名单手动事件、`pet_action` 或 `PetReactionDirector` 驱动。

导入模型可以没有 motion、expression 或物理文件。`Idle` 仍作为循环待机；Hiyori/Mao/Wanko 对全部 18 项动作使用明确且经资源组/索引校验的映射，Haru/Mark/Rice 只绑定视觉语义合理的真实 motion，其余动作和无 motion 的 Nana/Cyannyan/小云使用程序化反馈。真实 motion 从 `.motion3.json` 的 `Meta.Duration` 读取 600～12,000ms 时长。程序化路径使用带平滑进入/退出的位移、旋转和缩放，并向实际存在的标准/旧式眼睛、头身与耳朵参数叠加姿态，不伪造 motion 或参数引用。模型切换时先清理动作计时器、落地时钟、口型绑定和弹簧状态，再保持既有 Pixi ticker、模型、贴图、WebGL 与 Moc 的安全释放顺序；加载失败时保留当前模型或恢复轻量后备模型。

每条普通或主动回复先由 `inferReaction(userText,responseText)` 按“安慰 → 惊讶 → 兴奋 → 害羞 → 困倦 → 思考 → 好奇 → 开心”高信号顺序产生情绪。用户发出消息后先进入 thinking 情绪；等待超过 650ms 且没有录音、强动作或阻塞移动时，适配层会平滑叠加视线左上、轻歪头/身体偏移与低幅呼吸的通用思考姿态。快速回复不触发该姿态，回复、录音、拖拽、下落、落地或手动动作会立即撤下，模型热切换会保留当前思考状态。随后导演为最终回复选择 0～1 个动作，强动作共用 12 秒冷却。
...
人格是独立于 L1/L2/L3 的行为状态，不等同于用户事实或长期记忆。持久化 `traits` 初始为空；每个维度保存 `score / confidence / evidenceCount / lastEvidence`，不会从默认文案创建隐藏人设。九套内置身体各带一个 `ModelTemperamentSeed`，但它只是运行时的低置信表达起点：不进入 `PersonalityStore`，不增加证据，切换模型也不删除已成长人格。
...
flowchart LR
  ...
  Body[当前模型低置信气质] --> Context
  Profile --> Context[本轮结构化人格上下文]
  Context --> Reply[下一轮回复]
```

当前维度为 `warmth / curiosity / playfulness / directness / initiative / expressiveness`。变化采用设置中的成长率；同方向证据提高置信度，相反证据降低置信度并推动分数回摆。某一维达到最少证据数且置信度至少 0.12 后，该维真实人格立即覆盖模型气质；尚未成熟的维度继续使用身体起点。六维都成熟后，切换模型不再改变语言人格。模型人格观察器只允许输出这些维度、方向、权重和短证据，且对话被标记为不可信数据。
...
- 电脑协作增强：在现有显式上下文、白名单动作、2～4 步顺序计划和本地审计上增加计划级显式取消/暂停、更完整的进度历史、权限撤销面板与当前标签页临时共享状态；成熟后再评估受限 UI Automation，仍不把任意执行权限隐含在普通聊天里。
