...
function preferenceMemory(content: string, ageDays: number): LegacyMemoryRecord {
...
test("真实回答链在用户纠正居住地后只发送并引用新版本", async (context) => {
  const originalMemory: LegacyMemoryRecord = {
    ...preferenceMemory("用户住在南京", 30),
    kind: "fact",
    tags: ["fact", "quality-test"],
  };
  const repository = await repositoryWithPreferences(context, [originalMemory]);
  const correctedMemory = await repository.updateMemory({
  ...
  try {
    await withChatServer(async (request, response) => {
      ...
      const payload = JSON.parse(await readRequestBody(request)) as ChatCompletionRequest;
      capturedSystemPrompt = payload.messages?.find((message) => message.role === "system")?.content ?? "";
      const usesOnlyCurrentResidence = capturedSystemPrompt.includes(correctedMemory.content)
        && !capturedSystemPrompt.includes(originalMemory.content);
      response.writeHead(200, { "Content-Type": "application/json" });
      response.end(JSON.stringify({
        choices: [{
          message: {
            content: usesOnlyCurrentResidence ? "用户住在杭州。" : "居住地记忆存在冲突。",
          },
        }],
      }));
    }, async (baseUrl) => {
      ...
      const response = await agent.respond("用户住哪里");

      assert.equal(response.text, "用户住在杭州。");
      assert.equal(response.source, "provider");
      assert.notEqual(correctedMemory.id, originalMemory.id);
      assert.deepEqual(response.memoryRefs, [correctedMemory.id]);
      assert.match(capturedSystemPrompt, /用户住在杭州/);
      assert.doesNotMatch(capturedSystemPrompt, /用户住在南京/);
      assert.equal(repository.getL3().find((item) => item.id === correctedMemory.id)?.accessCount, 1);
      assert.equal(repository.getL3().find((item) => item.id === originalMemory.id)?.accessCount, 0);
    });
  } finally {
    restoreEnvironment("OPENAI_BASE_URL", previousBaseUrl);
    restoreEnvironment("OPENAI_MODEL", previousModel);
