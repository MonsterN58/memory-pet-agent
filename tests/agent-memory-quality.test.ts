...
test("模型舞台提示在写入 L1 和驱动情绪前转成自然口语", async (context) => {
  ...
  try {
    await withChatServer(async (_request, response) => {
      response.writeHead(200, { "Content-Type": "application/json" });
      response.end(JSON.stringify({
        choices: [{ message: { content: "（轻笑）被你发现了，*歪头*这下更自然啦。" } }],
      }));
    }, async (baseUrl) => {
      const settings: AgentSettings = structuredClone(DEFAULT_SETTINGS);
      settings.provider = { enabled: true, baseUrl, model: "natural-dialogue-test", temperature: 0 };
      const settingsStore = {
        get: () => structuredClone(settings),
        getApiKey: async () => "natural-dialogue-key",
        providerConfigured: async () => true,
      } as unknown as SettingsStore;
      const personality = {
      ...
      const response = await agent.respond("你刚才是不是笑了？");

      assert.equal(response.text, "哈哈哈，被你发现了，这下更自然啦。");
      assert.equal(response.emotion, "happy");
      const savedReply = memory.snapshot().l1.findLast((item) => item.role === "assistant");
      assert.equal(savedReply?.content, response.text);
      assert.doesNotMatch(savedReply?.content ?? "", /（轻笑）|歪头/);
    });
  } finally {
    restoreEnvironment("OPENAI_BASE_URL", previousBaseUrl);
    restoreEnvironment("OPENAI_MODEL", previousModel);
