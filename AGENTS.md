...
| 大模型 | 已完成陪伴增强版 | OpenAI 兼容端点、有限陪伴契约、倾诉/闲聊/信息/回忆节奏、真实 L1 角色轮次、自然记忆背景、人格连续性、非技术化离线降级 | 流式输出、多供应商适配、长期陪伴质量评测、重试/限流 |
| 电脑协作 | 已完成受控 MVP | 默认关闭的总权限、Chrome/Edge MV3 网页右键解释/总结/记住/聊聊、回环令牌桥接、剪贴板快捷键/右键、原生文本文件选择、4 项固定参数工具、仅本次/会话/始终/拒绝、执行前预览、500 条本地审计、网页提示注入隔离 | 多步骤可取消计划、权限撤销中心、标签页共享指示、本地待办/提醒、受限 UI Automation 评估 |
...
| 测试与安全 | 基础完成 | 121 个核心测试、20 例记忆质量 fixture、陪伴/记忆/人格、电脑规划与审批、桥接令牌/Origin/预检/大小、网页上下文隔离与明确记忆、Electron/语音/三模型 smoke、IPC 白名单、ASR/TTS/Live2D 资源边界、npm 0 漏洞 | 长期陪伴人工评测、浏览器扩展手工 E2E、麦克风硬件 E2E、更多数据迁移测试、安装包测试 |

最近一次已验证：

- `npm run typecheck` 通过。
- `npm test`：121/121 通过；新增覆盖电脑动作白名单规划、仅本次/会话/始终/拒绝、执行阶段二次权限检查、长期权限保存失败、五分钟预览过期审计、浏览器令牌/Origin/CORS 预检/输入边界、网页提示注入隔离和网页右键明确记忆；陪伴、三级记忆、人格、语音、TTS、Live2D 与模型资源回归继续通过。
...
- 390×700 本地生产 Renderer 下已检查电脑协作开关、桥接状态、四项权限策略、三项扩展操作和空审计布局；设置提交后桥接状态正确刷新。320×460 下已检查操作预览、四种授权按钮、执行后收起与 Live2D 同源资源加载，控制台 0 错误。验收图为 `output/playwright/computer-settings-390x700.png` 与 `output/playwright/computer-action-card-320x460.png`。
...
4. 主进程先用确定性规划器检查明确的白名单工作意图；它只生成固定参数预览，不执行。`AgentService.respond()` 把用户消息写入 L1，并按当前、历史或比较查询视图检索相关记忆；当前消息只保留一个副本。
5. 模型配置完整时，最近 L1 以真实 `user / assistant` 轮次传入；人格连续状态与时态门控后的 L2/L3 作为不可执行背景进入陪伴契约。失败或未配置时使用不暴露技术模式的本地陪伴回复。
...
普通聊天只规划 `open-url / copy-text / save-text-file / launch-app`。主进程保存真实参数并给 Renderer 随机 UUID；确认 IPC 只允许 UUID 加 `allow-once / allow-session / allow-always / deny`。保存文件始终仅本次授权并再走原生保存对话框，应用只允许记事本、计算器和资源管理器。每项操作执行前重新读取权限，结果进入 `computer-access.json`。
...
8. 所有用户记忆、人格证据和设置默认写本机；摄像头、屏幕、云同步或新增工具执行必须有显式授权。电脑工具真实参数保留在主进程，Renderer 只能回传操作 ID 和授权决定；网页内容不得进入工具规划器。
