...
| 心跳 | 已完成关系/感知增强版 | startup/scheduled/manual、一次性授权感知、迁移、人格/关系复盘、整理、结构化私有思考、开口决策和无图片审计 | 自适应频率、思考历史可视化、长期评测 |
| 主动聊天 | 已归并到心跳 | 唯一心跳出口、空闲阈值、冷却、安静时段、每日上限、具体话题价值判断、下一轮欢迎/拒绝反馈学习 | 更细的用户打扰反馈、情境长期评测 |
| 人格成长 | 已完成基础版 | 初始空白、六维连续状态、逐轮本地证据、心跳模型/离线复盘、置信度与相反反馈、独立持久化、设置页展示与重置 | 更丰富的隐式反馈、人格版本历史、用户纠错单项证据 |
| 关系成长 | 已完成基础版 | 独立用户理解、共同经历、关心方式、四阶段关系、来源/置信度/冲突更新、主动话题反馈、粗粒度活动习惯、设置页展示与重置 | 单项纠错/删除、关系版本历史、长期人工评测 |
| 大模型 / Agent tools | 已完成受控工具版 | OpenAI 兼容 `tools/tool_calls/tool_call_id`、最多四轮工具循环、记忆搜索/明确保存、人格/关系只读、按需桌面感知、四项电脑预览、桌宠动作、多模态 image_url、端点兼容降级、可见工具 trace | 流式输出、工具能力预探测、多步骤可取消计划、长期陪伴质量评测、重试/限流 |
| 桌面感知 | 已完成授权心跳版 | 屏幕/进程独立默认关闭、非启动心跳瞬时截图、鼠标所在显示器、图片不落盘、固定 tasklist 参数、窗口标题丢弃、已知活动分类与新启动基线 | 临时共享指示、本地视觉模型、跨平台进程适配 |
...
| 测试与安全 | 基础完成 | 144 个核心测试、20 例记忆质量 fixture、标准 tool_calls 往返、原话绑定记忆、截图附件隔离、电脑草稿二次清洗、Agent 动作优先级、陪伴/人格/关系/心跳、对话贴片、Electron/语音/三模型 smoke、IPC 白名单、npm 0 漏洞 | 长期陪伴人工评测、真实多模态/工具端点 E2E、浏览器扩展/麦克风硬件 E2E、更多数据迁移测试、安装包测试 |

最近一次已验证：

- `npm run typecheck` 通过。
- `npm test`：144/144 通过；新增覆盖标准 function tool 请求/解析/多轮回填、`memory_store` 用户原话绑定、`desktop_observe` 图片字节隔离、模型电脑草稿二次清洗、审批预览、Agent 指定动作与强动作冷却；关系、心跳、对话贴片、电脑能力、三级记忆、人格、语音、TTS、Live2D 与模型资源回归继续通过。
...
src/main/
  ...
  desktop-awareness-service.ts 授权截图短时载荷、tasklist 解析、可见应用分类与进程基线
  ...
  provider/
    openai-compatible.ts   文本/多模态/assistant tool_calls/tool 消息兼容客户端
...
6. 模型返回 assistant `tool_calls` 后，`AgentToolRuntime` 校验 JSON 并调用记忆、人格/关系只读、桌面感知、电脑预览或动作能力；结果用匹配 `tool_call_id` 回填，最多四轮。屏幕图片只作为一次性 image_url 附件，不进入工具文本。
...
所有主动话题只从心跳产生。完整顺序为：授权式短时感知 → L1→L2 → 人格复盘 → 关系复盘/粗粒度活动累计 → L2→L3 → `HeartbeatThought` → 策略与话题价值判断 → 至多一条主动回复 → 无图片心跳审计。手动心跳可绕过定时限制，但思考仍可选择安静；网页/剪贴板/文件共读是用户触发的响应，不属于主动聊天。

人格与关系分别保存已复盘 L2 ID，避免重复强化同一段对话。有模型时二者使用不同的受限 JSON schema，无模型时使用各自本地规则。屏幕截图只在非启动心跳、独立开关和图像模型配置完整时采集；进程检测只调用固定 `tasklist.exe /fo csv /nh /v`，窗口标题仅用于可见性判断后立即丢弃。
...
13. Agent function tool 名称和 schema 是稳定主进程契约。模型参数始终不可信：记忆写入绑定用户明确原话，人格/关系只读，桌面图片不进入工具 JSON，电脑工具只生成一个待审批预览，网页共读专用调用不携带 tools。新增工具必须同步定义、Runtime 校验、trace、测试和文档。
...
- 屏幕理解依赖用户配置的 Chat Completions 模型支持 `image_url`，当前没有本地视觉模型；普通聊天的 `desktop_observe` 或非启动心跳都可能在开关开启后使用一次性画面。图片不落盘，但用户看得见的派生回复会进入 L1。进程检测仅适配 Windows `tasklist`，粗粒度类别可能遗漏或误判当前活动。
...
- 模型客户端当前支持非流式 Chat Completions、标准 function tools 和 image_url；尚无流式 token/tool event、工具能力预探测或跨供应商 Responses API 适配。部分只兼容文本的端点会自动退到普通聊天模式。
