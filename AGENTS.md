...
> 最后更新：2026-07-14｜项目版本：0.1.0｜阶段：MVP 已可运行，进入能力增强与生产化阶段
...
| 检索 | 已完成增强版 | 中文单字/双字词法相关度、高频虚词抑制、最低文本证据门槛、重要度/时间/访问强化、召回评分拆解、20 例五类质量契约 | 自动更新/冲突语义、回复级评测、Embedding 与混合检索 |
...
| 测试与安全 | 基础完成 | 44 个核心测试、20 例记忆质量 fixture、类型检查、Electron smoke、三模型热切换 smoke、CSP、IPC 白名单、记忆修改输入边界/持久化/并发写/过期整理隔离/评分解释、人格空白/成长/冲突/去重、本地 ASR 输入边界、本地/云端 TTS 隔离、模型资源边界、npm 0 漏洞 | 回复级记忆质量评测、麦克风硬件 E2E、更多数据迁移测试、安装包测试 |

最近一次已验证：

- `npm run typecheck` 通过。
- `npm test`：44/44 通过，包含三级记忆迁移、20 例偏好更新/事实冲突/跨天跟进/持久化提示注入/用户纠错质量 fixture、零相关召回与访问强化隔离、主动聊天通用类型召回、L2/L3 修正与删除、磁盘重载、并发写队列、心跳过期提炼隔离、修改输入边界、可解释检索评分、设置迁移、人格成长/修正/心跳去重、本地 ASR PCM 输入边界/模型缺失状态/16k 下采样、Chromium 兼容模式隔离、本机 TTS 不访问云端、云端失败回退、聊天/TTS 端点隔离、TTS 鉴权/错误/12MB 上限，以及 Live2D 资源与导入边界。
- `npm run test:memory-quality`：5/5 类别、20/20 fixture 通过；完全使用临时本地 JSON 和真实 Repository，不访问网络或模型。
- `npm run build` 通过。
...
src/main/
  ...
  desktop-movement-controller.ts  失焦漫游、鼠标拖拽、重力落地、边界和动作方向
  preload.ts               contextBridge 白名单；渲染层唯一主进程入口
  agent-service.ts         结构化人格上下文、聊天、主动消息、反思、L2 与人格证据提炼
  heartbeat-service.ts     心跳调度、主动聊天限制与审计
  local-asr-service.ts     项目模型状态、PCM 边界、worker 生命周期与识别超时
...
4. `AgentService.respond()` 把用户消息写入 L1，检索相关记忆并生成回复。
5. 模型配置完整时，把已达到证据门槛的人格连续状态与相关记忆放入本轮系统上下文；失败或未配置时使用本地回复。
6. 助手回复写入 L1，`PersonalityEngine` 从本轮用户表达中小幅更新证据，变化从下一轮回复开始生效。
7. 渲染层更新当前字幕和模型表情；自动朗读默认直接使用本机 `speechSynthesis`，不经过 IPC 或外部服务。
8. 只有 `ttsMode=cloud` 时才通过 `voice:synthesize` 请求主进程；主进程使用独立 TTS 配置调用 `/audio/speech`，失败后渲染层回退本机语音且不影响文字回复。
...
1. **记忆更新与冲突质量**：在已建立的 20 例检索基线上增加回复级偏好遵循、自动新旧值识别、过期值泄漏指标和用户可见冲突标记；完成前不直接引入 Embedding/SQLite。
...
- 中文检索是带最低文本证据门槛的轻量词法算法，没有语义向量；它能抑制零相关与高频虚词误召回，但同义表达、代词指代和自动冲突消解能力仍有限。
...
- 当前 VAD 在 Renderer 中判断结束后一次性提交整段 PCM，尚未显示部分识别文本；首次使用会有约数秒模型加载时间，之后复用 worker。真实麦克风仍需在目标设备进行硬件 E2E。
...
- 拖拽允许在当前显示器工作区内放置宠物，松手后会落到该显示器底部；自主漫游仍是当前工作区内的水平移动。
