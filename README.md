...
- 记忆检索：综合文本相关度、重要度、时间衰减和访问频率排序；中文检索会削弱高频虚词干扰，并在缺少最低文本证据时返回空结果，避免无关记忆进入聊天上下文或被访问次数反复强化。
...
`npm run test:memory-quality` 会单独运行 20 个手写中文记忆 fixture，覆盖偏好更新、事实冲突、跨天跟进、持久化提示注入和用户纠错。它完全使用临时本地 JSON 和真实 Repository，不调用模型或网络；常规 `npm test` 也会包含这组质量契约。
...
搜索和聊天上下文不会再用重要度或新鲜度补齐完全无关的记录：持久记忆至少要达到加权文本相关度 `0.75` 才参与排序和访问强化。记忆类型带有本地化检索词，因此主动聊天仍能用“近期偏好、重要的事、待跟进话题”这类通用关注点找到候选；确实没有匹配时搜索返回空列表，本地聊天会明确表示尚未找到足够确定的记忆，主动聊天则沿用普通问候。该规则仍是轻量词法检索，不等同于同义词或指代消解。
