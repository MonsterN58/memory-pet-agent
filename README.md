...
- 语音交互：默认使用项目内 sherpa-onnx + 小型中文 Zipformer 模型离线识别，启动后非阻塞预热；识别阶段可再次点击麦克风立即取消，超时或 worker 故障会自动收尾并在下次请求重建。朗读默认使用本机已安装语音，可选 OpenAI 兼容 `/audio/speech` 云端 TTS，失败时自动回退本机语音。
...
- 说完保持约 1.2 秒安静会自动结束录音，也可再次点击麦克风手动结束；8 秒内没有检测到语音会给出明确提示。应用启动后会在后台非阻塞预热模型，不延迟窗口、文字聊天、记忆或心跳；同一 worker 会在后续识别中复用。
- 进入“正在本地识别…”后再次点击麦克风会取消本轮操作，立即清空状态并忽略迟到结果，不会直接开始下一轮录音。识别状态每秒显示已等待时间；主进程模型初始化和单次交互识别各有 30 秒上限，Renderer 另有 35 秒看门狗。取消、超时、worker 退出或同步提交失败都会终止当前 worker，下一次识别重新初始化。
