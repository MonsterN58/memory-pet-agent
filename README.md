...
- 陪伴式对话：不会把每轮当成问答工单；会区分倾诉、闲聊、信息请求和回忆确认，先回应情绪与意味，不强制追问或列清单，也不会假装全知。最近对话保留真实角色顺序，长期记忆只在相关时自然融入。
...
- 受控工作工具：能识别“打开网页/搜索、复制到剪贴板、保存文本文件、启动记事本/计算器/资源管理器”等明确请求，先显示固定参数预览，再按“仅本次 / 本次会话 / 始终允许 / 不做”授权；不向模型开放任意 Shell、任意文件系统或后台鼠标键盘控制。
